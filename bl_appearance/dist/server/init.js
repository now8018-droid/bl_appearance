...
async function saveAppearance(src, frameworkId, appearance, force) {
  if (!force && src && frameworkId && getFrameworkID(src) !== frameworkId)
    ...
    migrate = async (src) => {
      ...
      for (const element of response) {
        if (element.skin) {
          ...
          await saveAppearance(playerSrc, element.citizenid, response2, true);
        }
      }
...
async function getOutfits(src, frameworkId) {
  ...
  try {
    let response = await import_oxmysql.oxmysql.prepare("SELECT * FROM outfits WHERE player_id = ? OR (jobname = ? AND jobrank <= ?)", [frameworkId, job.name, job.grade.name]);
...
core.RegisterUsableItem(outfitItem, async (source2, slot, metadata) => {
  ...
  if (player?.removeItem(outfitItem, 1, slot))
    emitNet("bl_appearance:client:useOutfitItem", source2, metadata.outfit);
});
